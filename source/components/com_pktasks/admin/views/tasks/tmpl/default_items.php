...
for ($i = 0; $i != $count; $i++)
{
    ...
    if ($can_edit || $can_edit_own) {
        $title = '<a href="index.php?option=com_pktasks&task=task.edit&id=' . intval($item->id) . '">'
    ...
    // Check-in button
    if ($item->checked_out) {
        $btn_checkin = JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'tasks.', $can_checkin);
    }
    else {
        $btn_checkin = '';
    }
    ...
    // Item priority
    $prio_disabled = $can_edit_state ? '' : ' disabled';

    $priority = '<i id="prio-' . $item->id . '"'
              . 'class="icon-flag priority prio-' . intval($item->priority) . $prio_disabled . '"'
    ...
    // Format progress bar
    if ($progress_type == 1) {
        // Simple Todo/Done progress
        if ($item->progress == 100) {
            $progress_class = 'success';
            $progress_text  = $txt_done;
        }
        else {
            $due_time = floor(strtotime($item->due_date) / 86400) * 86400;
        ...
        $progress = '<a id="progress-' . $i . '" class="span12 task-progress btn btn-' . $progress_class .  '"'
                  . ' data-id="' . $item->id . '" data-progress="' . $item->progress . '" data-due="' . strtotime($item->due_date) .  '">'
                  . $progress_text
                  . '</a>';
    }
    else {
    ...
    <tr class="row<?php echo ($i % 2); ?>" sortable-group-id="<?php echo $item->project_id; ?>">
        ...
        <td class="center">
            <div class="btn-group">
                <?php
                    echo JHtml::_('jgrid.published', (int) $item->published, $i, 'tasks.', $can_change, 'cb')
                    . $actions;
                ?>
            </div>
        </td>
